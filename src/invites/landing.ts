...
export function landingHtml(opts: {
  ...
  const { couple, dateISO, venue, mapsUrl, dressCode, cbu, alias, rsvpEndpoint, slug } = opts;
...
async function rsvp(st){await fetch("${rsvpEndpoint}",{method:"POST",headers:{"Content-Type":"application/json"},
body:JSON.stringify({slug:"${slug}",status:st,dietary:document.getElementById("diet").value})});alert("¡Gracias!")}
